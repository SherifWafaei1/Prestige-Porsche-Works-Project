    ...
    function updateAuthDisplay() {
        ...
        if (localStorage.getItem('isAdminLoggedIn') === 'true') {
            ...
            if (cartBtn) cartBtn.style.display = 'none'; // Ensure cart button is hidden for admin
            if (adminControls) {
            ...
            if (cartBtn) cartBtn.style.display = 'none'; // Hide cart for guests
            if (adminControls) adminControls.style.display = 'none';
            if (userControls) userControls.style.display = 'none';
            if (adminDashboardMenuItem) adminDashboardMenuItem.style.display = 'none';
    ...
    showSection = function(sectionId, pushState = true) {
        ...
        if (activeSection) {
            ...
            if (pushState) {
                ...
                if (sectionId) {
                    const sectionHeader = activeSection.querySelector('.section-header h2');
                    if (sectionHeader) {
                        title = `Prestige Porsche Works - ${sectionHeader.textContent}`;
                    } else {
                        title = `Prestige Porsche Works - ${sectionId.charAt(0).toUpperCase() + sectionId.slice(1)}`;
                    }
                }
    ...
    function toggleMenu() {
        ...
        if (menu.style.display === 'block') {
            ...
            // Add event listener for outside click, with a small delay to avoid immediate close
            setTimeout(() => {
    ...
    function showSection(sectionId, pushState = true) {
        ...
        if (activeSection) {
            activeSection.style.display = 'block';
            // Store active section in local storage for persistence
            localStorage.setItem("activeSection", sectionId);
            ...
            if (pushState) {
                let title = document.title;
                if (sectionId) {
                    // Try to get a more descriptive title from the section's header
                    const sectionHeader = activeSection.querySelector('.section-header h2');
                    if (sectionHeader) {
                        title = `Prestige Porsche Works - ${sectionHeader.textContent}`;
                    } else {
                        // Fallback title if h2 not found in the section
                        title = `Prestige Porsche Works - ${sectionId.charAt(0).toUpperCase() + sectionId.slice(1)}`;
                    }
                }
                history.pushState({ section: sectionId }, title, `#${sectionId}`);
            ...
            // Special handling for the payment section
            if (sectionId === 'payment') {
                switchPaymentMethod('card');
            }
            
            // Minimize the menu after showing the section
            const menu = document.getElementById('menu');
            if (menu) {
                menu.style.display = 'none';
                document.removeEventListener('mousedown', handleMenuOutsideClick);
    ...
    // New function to toggle 'Show More/Show Less' content
    function toggleAboutContent() {
        ...
        if (moreContent.style.display === 'none' || moreContent.style.display === '') {
            ...
            toggleButton.innerHTML = '<i class="fas fa-minus-circle"></i> Show Less';
            // Smooth scroll to the button after expanding
            toggleButton.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } else {
            moreContent.style.display = 'none';
            toggleButton.innerHTML = '<i class="fas fa-plus-circle"></i> Show More';
        }
    }

    // Call this function when the page loads to ensure initial state is correct (content hidden)
    document.addEventListener('DOMContentLoaded', () => {
        aboutUs(); // Ensure the content is hidden on load
    });
...
document.head.appendChild(style);
    ...
    async function updateModelStock(modelId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                await fetchModels(); // Ensure models array is updated with latest stock
    ...
    function updateAuthDisplayWithUser(user) {
        ...
        if (user && user.role === 'admin') {
            if (loginBtn) loginBtn.style.display = 'none';
            if (registerBtn) registerBtn.style.display = 'none';
            if (cartBtn) cartBtn.style.display = 'none'; // Hide cart button for admin
            if (adminControls) {
                adminControls.style.display = 'flex';
                const welcomeMessage = adminControls.querySelector('.welcome-message');
            ...
            if (userControls) {
                userControls.style.display = 'flex';
                const welcomeMessage = userControls.querySelector('.welcome-message');
                if (welcomeMessage && user && user.firstName) {
                    welcomeMessage.textContent = `Welcome, ${user.firstName}!`;
                } else if (welcomeMessage) {
                    welcomeMessage.textContent = 'Logged in';
                }
    ...
    function showOrderHistory() {
        ...
        fetch(`http://localhost:3001/api/orders/user/${encodeURIComponent(currentUser.email)}`)
            ...
            .then(orders => {
                if (!orders || orders.length === 0) {
                    content.innerHTML = '<p>No orders found. Create your first order here: <a href="#models" onclick="showSection(\'models\')">Select Model</a></p>';
                    return;
                }
...
html += '</tbody></table>';
    ...
    // Initial call to update authentication display on page load
    updateAuthDisplay();

    // Logo sound fade effect
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fadeIn() {
            ...
            fadeInterval = setInterval(() => {
                if (logoSound.volume < 0.9) {
                    logoSound.volume += 0.1;
                } else {
                    logoSound.volume = 1;
                    clearInterval(fadeInterval);
                }
        ...
        logo.addEventListener('mouseenter', fadeIn);
        logo.addEventListener('mouseleave', fadeOut);
    });

    // Handle browser back/forward buttons
    window.addEventListener('popstate', (event) => {
        if (event.state && event.state.section) {
            showSection(event.state.section, false); // false to prevent adding another history entry
        } else {
            // Fallback to home if no specific section in state (e.g., initial load or external navigation)
            showSection('home', false);
        }
    });

    // Initial page load: check URL hash and display section
    document.addEventListener('DOMContentLoaded', () => {
        const initialSection = window.location.hash.substring(1); // Remove the #
        if (initialSection && document.getElementById(initialSection)) {
            showSection(initialSection, false); // Show the section from URL, don't push state
        } else {
    ...
    // Function to show admin sections and scroll to the content
    function scrollToAdminSection(sectionId) {
        showAdminSection(sectionId); // First, show the requested admin section
        const adminContentDiv = document.getElementById('admin-section-content');
        if (adminContentDiv) {
            adminContentDiv.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    }